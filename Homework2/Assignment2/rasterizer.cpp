...
{   
    ...
    float c1,c2,c3;
    c1 = AB[0] * AP[1] - AB[1] * AP[0];
    c2 = BC[0] * BP[1] - BC[1] * BP[0];
    c3 = CA[0] * CP[1] - CA[1] * CP[0];
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int x = min_x; x <= max_x; x++) {
        for (int y = min_y; y <= max_y; y++) {
            // 记录最小深度
            float minDepth = HUGE_VALF; // FLT_MAX;
            // 四个小点中落入三角形中的点的个数
            int count = 0;
            // 对四个小点坐标进行判断 
            for (int i = 0; i < 4; i++) {
                auto centerPoint = pos[i];

                if (insideTriangle((float)x + centerPoint[0], (float)y + centerPoint[1], t.v)) {
                    auto[alpha, beta, gamma] = computeBarycentric2D((float)x + centerPoint[0], (float)y + centerPoint[1], t.v);
                    float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                    float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                    z_interpolated *= w_reciprocal;
